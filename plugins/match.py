...
def command_match(m):
    ...
    if is_user(cid):
        ...
        for region in ['euw','eune','br','na','las','lan','kr','tr','ru','oce']:
            txt += '\n/match\_' + region + responses['match_3'][lang(cid)] + '*' + region.upper() + '*'
...
def get_match_info( invocador, region, cid):
    ...
    try:
        ...
        bot.send_chat_action( cid, 'typing')
        bot.send_message( cid, responses['summoner_error'][lang(cid)]%(invocador,region.upper()))
        return None
    ...
    for jugadores in partida['participants']:
        if jugadores['teamId'] == 100:
            azul[jugadores['summonerName']] == campeones[str(jugadores['championId'])]['name']
        else:
            rojo[jugadores['summonerName']] == campeones[str(jugadores['championId'])]['name']
    bot.send_chat_action( cid, 'typing')
...
def get_summoner_info_2( invocador, region, champion, cid):
    ...
    if summoner_level == 30:
        ...
        if 'rankeds' in locals():
            ...
            division = ''
            winrate = '-'
        txt = responses['summoner_30_2'][lang(cid)] % (summoner_name, liga, division, winrate, champion)
    else:
        txt = responses['summoner<30_2'][lang(cid)] % (summoner_name, summoner_level, champion)
    return txt
