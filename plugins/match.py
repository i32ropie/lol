...
@bot.message_handler( func=lambda message: message.text.split(' ')[0].split('@')[0] in ['/match_euw','/match_eune','/match_br','/match_na','/match_las','/match_lan','/match_kr','/match_tr','/match_ru','/match_oce'] )
def match_info(m):
    cid = m.chat.id
    uid = m.from_user.id
    if is_banned(uid):
